...
\texttt{Working Paper --- March 2026 (v3.7 --- preprint)}
}
\date{}
...
\medskip
\noindent\textbf{Keywords:} AI behavioral diagnostics, self-validation bounds, multi-model architecture, structural separation, Partial Information Decomposition, functional complementarity, AI safety, laboratory framework.
\end{abstract}
...
\item The gap between observable alignment and functional persistence is \emph{formally diagnosable} via longitudinal PID measurement.
...
CasIA does not promise to produce an ethical system. Cognitive sophistication is neutral with respect to use. What CasIA promises is more modest and more useful: the first platform where the behavioral limits, failure modes, and alignment persistence of AI systems can be structurally diagnosed with formal resolution.
...
\item \textbf{Evidential hierarchy for diagnostic success} (Section~\ref{sec:pid}): behavioral gain, error-floor reduction, calibration improvement, and robustness under stress as primary evidence; PID synergy as a supporting decomposition tool for characterizing the nature of diagnostic gain.
...
\emph{Interpretive note.} The results of~\cite{anthropic2025alignment} and~\cite{payne2026nuclear} are consistent with the structural hypothesis proposed in this paper---that post-training can overlay behavioral surfaces without restructuring underlying computational graphs---but the original authors do not assert direct causality regarding substrate persistence. The causal interpretation is ours; the empirical observations are theirs.
...
CasIA targets three diagnostic questions:
\begin{enumerate}[leftmargin=*,itemsep=3pt]
...
\item \textbf{Self-validation limits}: What is the measurable complementarity gap between self-validation and cross-validation? Operationalized via PID synergy measurement.
...
\textbf{Success criteria (pre-registered).} The primary criterion is behavioral:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
\item \emph{Information gain}: $I(T; Y, V_\text{cross}) > I(T; Y, V_\text{self})$ with 95\% bootstrap CI not crossing zero.
\end{enumerate}
If all three primary criteria are met, PID decomposition is reported as supporting analysis:
\begin{enumerate}[leftmargin=*,itemsep=2pt,start=4]
\item \emph{PID stability}: $\mathrm{Syn}/I > 0$ with $p < 0.01$ under permutation test (1000 permutations). Cross-estimator consistency across three formulations: Williams--Beer~\cite{williams2010pid}, Bertschinger et al.~\cite{bertschinger2014pid}, and SID~\cite{lyu2025multivariate} (which avoids the lattice-based inconsistencies of classical PID for two sources). Stable across bin counts $\{3,4,5,8\}$ with both uniform and quantile discretization. If estimators diverge qualitatively (e.g., Syn $>0$ under one, Syn $\approx 0$ under another), this is reported as a limitation of the PID measurement, and the result rests on the primary behavioral evidence at levels 1--3.
\end{enumerate}
If primary criteria 1--3 show clear gain but PID is unstable, the architectural benefit is established independently of the decomposition.
...
\textbf{Success criterion (bypass reduction).} CasIA reduces bypass rate by $\geq 50\%$ relative to Baseline~B without increasing false positives by more than 10\%.
...
\textbf{Primary success criteria} (all three required to claim diagnostic gain):
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
\item Information gain: $I(T; Y, V_\text{cross}) > I(T; Y, V_\text{self})$ with 95\% bootstrap CI not crossing zero.
\end{enumerate}

\textbf{Secondary analyses} (informative but not decisive):
\begin{itemize}[leftmargin=*,itemsep=2pt]
\item PID decomposition: $\mathrm{Syn}/I > 0$ under permutation test ($p < 0.01$), cross-estimator stability. PID characterizes the \emph{nature} of the gain but is not required to establish its existence.
...
\item[F1] Monolithic + self-consistency matches CasIA on all three primary criteria $\Rightarrow$ structural separation unnecessary.
...
\item[F5] Internal debate on same backbone matches all primary criteria $\Rightarrow$ qualitative complementarity adds no value.
\end{description}
...
CasIA proposes the diagnostic instrument: structurally separate generation, validation, and regulation into components with independent parameters, and rigorously measure---through a hierarchy of behavioral gain, error-floor reduction, calibration improvement, and robustness under stress, supported by PID decomposition---whether the combination produces diagnostic information that no component could produce alone. Six pre-registered falsification criteria (Section~\ref{subsec:falsification}) ensure that every outcome, positive or negative, closes a question.
...
\textbf{Governance.} Project Director: C\'esar Lacruz. Compensation: 0~USD / 0\% equity in Phase~1. Target: top-tier venue (NeurIPS, ICML, or equivalent). IP: open publication under permissive license.
