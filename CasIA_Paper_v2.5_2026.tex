...
\author{
  ...
  \small Working Paper --- March 2026 (v2.5 -- preprint)
}
...
\begin{enumerate}
  ...
  \item \textbf{Computational asymmetry:} Component $\mathcal{V}$ is allocated at least as much effective computational depth as $\mathcal{G}$. This is not a defect but a design requirement: deep validation requires sufficient computation to capture errors invisible to the generator. A specialized, efficient validator may achieve this depth with lower wall-clock latency than $\mathcal{G}$. We formalize this as a design principle in Section \ref{sec:latency} (Principle \ref{princ:asymmetry}).
...
\begin{remark}[Conflict Resolution Between $\mathcal{V}$ and $\mathcal{N}$]
\label{rem:conflict}
When $\mathcal{V}$ assigns a high coherence score $v_t$ but $\mathcal{N}$ issues a \texttt{reject} or \texttt{modify} action (e.g., the output is factually coherent but normatively unsafe), the regulator's decision takes precedence. This is by design: safety is a hard constraint, quality is an objective to maximize subject to that constraint. The regulator $\mathcal{N}$ is architecturally \emph{inapelable} within a given cycle---analogous to a constitutional court that can override legislative output. However, the system avoids degenerate convergence to trivial outputs (abstention by exhaustion) through two mechanisms: (1) the regulator's output $\phi_t$ must contain a structured justification sufficient for $\mathcal{G}$ to modify its response directionally (see Remark \ref{rem:diagnostics}); and (2) the iteration counter $t$ imposes bounded cost, with safe failure (abstention with full audit log) as the explicit fallback. Experiment 2 includes a failure taxonomy that tracks regulatory deadlock---cases where $\mathcal{N}$ systematically rejects despite improving $v_t$---as a measurable pathology.

Two additional design choices merit explicit justification:

\emph{Channel independence.} As specified in Algorithm \ref{alg:protocol}, $\mathcal{N}$ receives $(x, y_t, v_t)$ but \emph{not} the validator's diagnostics $\sigma_t$. This is deliberate: if $\mathcal{N}$ received $\sigma_t$, its normative judgment would be contaminated by $\mathcal{V}$'s coherence assessment, breaking the independence of evaluation channels. The regulator must assess safety from its own representational space $\mathcal{R}_{\theta_N}$, not through the validator's lens. The scalar $v_t$ is included as a minimal coordination signal (to avoid rejecting outputs that $\mathcal{V}$ has not yet evaluated), not as a diagnostic input.

\emph{Semantics of \texttt{modify}.} The action $a_t = \texttt{modify}$ does not imply that $\mathcal{N}$ generates replacement text---that would make it a second generator and collapse the privilege separation. Rather, \texttt{modify} is operationally equivalent to \texttt{reject} with a directional diagnostic in $\phi_t$ (e.g., ``remove unsupported claim in paragraph 3'' rather than bare ``rejected for safety''). The generator $\mathcal{G}$ receives $\phi_t$ as part of the feedback context $c_t$ and attempts a new generation informed by the regulatory constraint. Generation remains exclusively the function of $\mathcal{G}$.
\end{remark}
...
\begin{remark}[Actionable Diagnostics as Design Requirement]
\label{rem:diagnostics}
The receptiveness parameter $\beta$ in Proposition \ref{prop:linear_contraction} depends critically on the \emph{quality} of the validator's feedback, not merely its existence. If $\mathcal{V}$ produces only a scalar score $v_t$ without diagnostic information, the generator may lack sufficient signal to correct its output, leading to oscillation between equally erroneous responses. To maximize $\beta$ in practice, the validator's output should include structured diagnostics $\sigma_t$ identifying \emph{which} aspects of $y_t$ are deficient and \emph{why}, rather than a bare accept/reject signal. Similarly, the regulator's output $\phi_t$ should contain a justification sufficient for $\mathcal{G}$ to modify its response directionally (e.g., ``factual claim X contradicts source Y'' rather than ``rejected for safety''). This is a design requirement on the implementation, not a modification of the theoretical bound---but it is the difference between a system that converges in 3 iterations and one that oscillates until timeout at $K$.
\end{remark}
...
For the CasIA system to produce non-trivial validations, the validator should be allocated at least as much effective computational depth as the generator:
\begin{equation}
  C_V \geq C_G
\end{equation}
where $C$ denotes effective computational budget (FLOPs, reasoning steps, or equivalent depth measure), not necessarily wall-clock latency. A compact, specialized validator may satisfy $C_V \geq C_G$ with $\tau_V < \tau_G$ if its architecture achieves higher computational density per unit time.
\end{principle}

\paragraph{Justification.} If $C_V < C_G$, the validator performs less computation than the generator. Since validation must evaluate the coherence of a complete output (not merely continue generation), its computational complexity is at least of the same order. A validator with less computational depth than the generator is necessarily performing a superficial evaluation, which reduces $\Unq(T; S_V \setminus S_G)$ and therefore the potential synergy.\footnote{Under idealized assumptions, restricting the validator to a fraction $c < 1$ of the generator's compute would bound $\MI(T; \mathcal{R}_{\theta_V}) \leq c \cdot \MI(T; \mathcal{R}_{\theta_G}) + \mathcal{O}(\log c)$. This heuristic motivates the principle but should not be taken as a formal result; the actual relationship between compute budget and information capacity is architecture-dependent.}
...
\begin{remark}
\label{rem:asymmetry}
This principle formalizes an intuition from cognitive systems design: \emph{deliberation} is intrinsically more computationally demanding than \emph{reaction}. This is analogous to Kahneman's System 1/System 2 distinction \cite{kahneman2011thinking}: fast thinking generates, slow thinking evaluates. However, the constraint is on computational depth, not wall-clock time: a specialized validator with an efficient architecture (e.g., a smaller model fine-tuned for evaluation tasks) may achieve sufficient depth faster than a general-purpose generator, provided its expressive capacity is adequate to capture errors in the generator's blind spots.
\end{remark}
...
Phase 0 is immediately executable: run two frontier models via public API on a FOLIO subset ($N = 200$), discretize outputs, compute PID, and report $\Syn/\MI$ with bootstrap CI. To disambiguate low synergy due to theoretical limitations from low synergy due to representational convergence, Phase 0 should include at least two pairings: (a) a within-family pair (e.g., two transformer-based frontier models from different labs) and (b) an architecturally diverse pair (e.g., transformer + state-space model, or neural + symbolic engine). If pairing (b) yields significantly higher $\Syn/\MI$ than pairing (a), the complementarity gap is attributable to representational convergence, not to a weakness in the theory. This produces the first empirical data point on the complementarity gap (Section \ref{sec:gap}) within two weeks at negligible cost. A positive signal justifies Phase 1 investment; a null signal across both pairings is itself informative and publishable.
